...
In this project we built a recommendation system to predict the rating of movies the users would rate them. Recommendation systems are a common challenge for data scientists. As modern tech firms use those recommendation systems to make the user experience better. On Amazon.org for example we find on the front page the products that we are probably interested in. The tech companies that use those recommendation systems like Netflix or Amazon have been extremely successful in recent years, which suggests that it could be good to create a better user experience by suggesting movies a customer likes. 

In the previous courses we already covered a subset of the original movielens dataset.
During this project we will explore the data and then try to apply the same methods we learned so far to this larger datasets.
Since the dataset is very large, we are limited to algorithms that also work with larger datasets in a reasonable amount of time. The goal is to built a movie recommendation system and than try to optimize the algorithms Root mean squared error.

## Root-mean-squares-error(RMSE)

The Root mean squared error is the average distance between the predicted value and the actual value:

$$RMSE = \sqrt{\frac{1}{N}\sum_{u,i} (\hat{Y}_{u,i}-Y_{u,i})^{2}}$$

## Key steps

We start with the data preparation where we import the data and prepare it for the analysis. 
Than we explore the data in the data exploration section.
At the end we explain the method, built the model and evaluate the model in the modeling section.

# Data preparation



First we import the data as shown in the description of the project:
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
train_set %>% group_by(userId,genres) %>% summarise(rating=mean(rating)) %>% ggplot(aes(genres,rating)) + geom_point() + geom_hline(yintercept = mean(train_set$rating),col="red",size=2)
...
The next plot shows the average rating for each month. By looking at the plot, we can see that the month the movie was rated has no influence on the rating of the movie.
...
Furthermore we can look at the year the movie was rated. We can see that the only year the  movies were rated different was the first year 1995. But apparently in year 1995 are not enough ratings. 
...
We start with the Naive mode, which predicts all ratings to be the average rating. We use this model as a reference to see, how much we can improve the predictive capabilities of our model.
...
RMSE(pred_mu , test_set$rating)

model_rmse <- data.frame(model="Naive model" ,RMSE  = RMSE(pred_mu, test_set$rating))
model_rmse
```
...
The userId model tries to account for the user bias, since as we saw in the data visualization section some user rate movies different than others. In order to calculate the user  bias we group the data set by userid and subtract the average rating from the average rating of each user. If the user bias is -1 than this means, that this user rates movies on average 1 point worse than the average movie rating. 

$$ Y = \mu + b_{u} + \epsilon_{u,i} $$
...
The userId+ movieId model tries to account for the user bias and movie bias, since as we saw in the data exploration section some movies are rated different than others, since some movies are just better than others. In order to calculate the movie bias we group the dataset by movieId and subtract the average rating and the user bias from the average rating of each movie.


$$ Y = \mu + b_{u} + b_{i} + \epsilon_{u,i} $$
...
The userId+ movieId model  tries to account for the user bias and movie bias, since as we saw in the data visualization section some movies are rated different than others, since some movies are just better than others. In order to calculate the movie bias we group the dataset by movieId and subtract the average rating and the user bias from the average rating of each movie.


$$ Y = \mu + b_{u} + b_{i} +  b_{g} + \epsilon_{u,i} $$
...
RMSE(pred_bu_bi_g , test_set$rating)

model_rmse[4,1]<-"userId + movieId + genres model"
model_rmse[4,2] <- RMSE(pred_bu_bi , test_set$rating)
model_rmse
```
...
Next we use regularization to punish movie ratings that were not rated as much. Since the a low number of ratings will lead to inaccurate predictions. For example, if a movie was only rated by one user and this user gave the movie a 5 star rating, we cannot conclude that this movie is in fact much better that a movie which  was rated 1000 times and only has a rating of 4. In order to punish predictions that were made on a small sample size we use lambda as an optimization parameter. As we can see in the plot bellow the optimal value for lambda is 15. 
...
When looking at the performance we can see that this method improved our model a bit. 

```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
model_rmse[5,2] <-  rmses[which.min(rmses)]
model_rmse[5,1] <-  "Regularization userId + movieId + genres model"
model_rmse
```
...
model_rmse[6,1]<-"Validation:Regularization userId + movieId + genres model"
model_rmse[6,2] <- final_model(validation,edx)
model_rmse
```
...
The goal of this project was to built a movie recommendation system, which predicts the ratings each user would make for each movie.We explored the  data to get insights  about the predictive power of the features. Than we used a machine learning technique that doesn't take a lot of computational power for large data set. Than we evaluated our models we built and choose a model for the validation set. Our final model is the "Regularization userId + movieId + genres model" and  achieved a RMSE of 0.8654897 on the validation set.
